...
> **⚠️ Token Usage:** A full pipeline run consumes roughly **4–6x** the tokens of single-agent Ralph implementations. Use a strong model (Claude Opus 4.6 etc) for **PRD, and Plan** agents where reasoning quality matters. Use a cheaper model (GPT-5 mini, etc.) for the **Ralph loop agent**, which only read state and review code.
